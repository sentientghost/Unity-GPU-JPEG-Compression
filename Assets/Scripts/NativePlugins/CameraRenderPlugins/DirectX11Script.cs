...
public class DirectX11Script : MonoBehaviour
{
    ...
    [DllImport("DirectX11")]
    private static extern float[] SetTextureFromUnity(System.IntPtr texture, int imageWidth, int imageHeight, float cameraQuality, string path);
    ...
    string ImageName(int imageHeight, int cameraQuality, int frameCount)
    {
        ...
        if (buildMode == "Editor")
        {
            ...
            return string.Format("{0}/../Images/Native Plugins/{1} Mode/{2} Scene/d3d11_{3}p_{4}_{5}.jpg", Application.dataPath, buildMode, SceneManager.GetActiveScene().name, imageHeight, cameraQuality, frameCount+1);
        }   
        else
        {
